        ...
        if (!file.exists()) {
            ...
            if (choice == 0) {
                try {
                    ...
                    JOptionPane.showMessageDialog(null, Data.error_modelDownloadFailed, "Error", JOptionPane.ERROR_MESSAGE);
...
//        }    
    ...
    public void populateProgressBar(double rating) {
        int r = (int) (rating * 100);
        jProgressBar_rating.setValue(r);
    }

    private void getNews(double rating) {
        setStatus(Data.newsDownloading);
        System.out.println(Data.newsDownloading);
        boolean downloaded = false;
        try {
            nameList = downloader.downloadImageSet(rating);
            System.out.println(Data.newsDownloaded);

            downloaded = true;
            setStatus(Data.newsDownloaded);

        } catch (JSONException ex) {
            setStatus(Data.newsDownloadFailed);
            //Logger.getLogger(Main.class.getName()).log(Level.SEVERE, null, ex);
            System.out.println(Data.error_newsImagesDownloadFailed);
    ...
    private void jMenuItem4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jMenuItem4ActionPerformed
        ...
        if (check) {
            setStatus(Data.modelUpdated);
            imageAnalizer = new ImageAnalyzer(Data.filePath_downloadNnet);
            jLabel_modelVersion.setText(downloader.getModelVersion());
            setStatus(Data.modelUpdated);
            System.out.println(Data.modelUpdated);
        } else {
            setStatus(Data.modelUpdateFailed);
        }
    ...
    private void clearNewsFeed() {
        ...
        jLabel_hospitalName.setVisible(false);
        jLabel_hospitalContact.setVisible(false);
        jLabel_hospitalName.setVisible(false);
    }
    ...
    private void setTextRating(double r) {
        String rating = Double.toString(r);
